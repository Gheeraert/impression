...
% \lateiTocAuthorPlain : copie de l'auteur sans le rendu petites capitales
% du nom de famille (vérification humaine directe du 2026-08-04, référentiel
% v0.7) : le nom de l'auteur doit apparaître bas de casse dans la TDM, alors
% que \lateiSignatureAuthor porte le rendu de la signature de fin d'article
% (§8), où le nom de famille EST en petites capitales (\textsc{Nom}, capturé
% tel quel depuis <hi rend="small-caps">). Calculée une fois, au moment de
% la capture (\lateiContributionAuthor ci-dessous), PAS au moment de
% l'écriture de l'entrée de TDM : \addcontentsline écrit son argument via
% \protected@write, qui \edef-développe tout ce qui n'est pas protégé —
% une redéfinition locale de \textsc placée directement dans cet argument
% (première tentative, abandonnée) ne s'exécute donc jamais normalement : un
% \edef ne peut pas EXÉCUTER les primitives non désarmables (\def, \global…)
% qu'un \renewcommand appelle en interne, il les recopie telles quelles —
% ce qui corrompait le fichier .toc plutôt que de simplement neutraliser
% \textsc (bug réel constaté par compilation : erreurs "\textsc has an
% extra }" ailleurs dans le document, bien après le point réellement fautif).
% \protected@xdef ci-dessous s'exécute en revanche au fil normal du
% document (pas dans un argument \addcontentsline), où une redéfinition
% locale de \textsc via un simple \def EST correctement exécutée par
% l'expansion.
\newcommand{\lateiTocAuthorPlain}{}
\newcommand{\lateiResetContributionSignature}{%
  ...
  \global\let\lateiTocAuthorPlain\lateiSignatureEmpty
}
...
% \protected@xdef (utilisé ci-dessous) contient un "@" : catégorie de code
% 11 requise pour le lire comme partie d'un seul nom de commande — comme
% pour \@empty (documenté plus haut pour \lateiSignatureEmpty), mais avec
% un piège de plus ici : un \makeatletter DANS le corps de la macro, exécuté
% seulement à l'usage de \lateiContributionAuthor, arrive trop tard — le
% corps de \newcommand{{...}} est tokenisé une fois pour toutes au moment où
% ce fichier est lu (\input), pas à chaque invocation ultérieure ; "@" doit
% donc déjà avoir la catégorie 11 AU MOMENT où \protected@xdef est LU dans
% cette définition, pas seulement au moment où elle s'exécute (bug réel
% rencontré : un \makeatletter placé à l'intérieur du corps produisait
% encore "You can't use a prefix with `the character @'", preuve que la
% tokenisation avait déjà eu lieu). D'où le \makeatletter englobant toute
% la définition ci-dessous, refermé par \makeatother juste après.
\makeatletter
