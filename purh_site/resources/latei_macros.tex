...
% ne pas les dupliquer ailleurs. Titre/sous-titre/auteur calibrés sur les
% valeurs observées (référentiel §2.5, §5.3) : Josefin Sans Thin 16 pt
% capitales centré pour le titre, Thin Italic 12 pt pour le sous-titre,
% corps courant gras centré pour l'auteur (style txt_auteur). Positions
% verticales exactes (mm) non calibrées ici — hors périmètre de cette passe.
\newcommand{\lateiContributionTitle}[1]{%
  \par\vspace{0.5\baselineskip}%
  {\PURHTitreFont\fontsize{16pt}{19pt}\selectfont\centering\MakeUppercase{#1}\par}%
...
\NewDocumentCommand{\lateiRenderHead}{m}{%
  \IfStrEq{\lateiHeadContext}{part}{\part*{#1}\lateiMarkBothVerso{#1}\addcontentsline{toc}{part}{#1}}{%
