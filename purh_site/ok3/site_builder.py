...
class SiteBuilder:
    ...
    def _finalize_build(
        ...
        staged_back_cover = self._copy_back_cover_asset(getattr(config, "back_cover_path", None), config.output_assets_dir)
        ...
        if staged_back_cover and back_cover_source == 'assets/quatrieme':
            back_cover_source = f'assets/quatrieme/{staged_back_cover.name}'
    ...
    def _copy_back_cover_asset(self, back_cover_path: Path | None, output_assets_dir: Path) -> Path | None:
        if back_cover_path is None or not back_cover_path.exists() or not back_cover_path.is_file():
            return None
        back_cover_dir = output_assets_dir / 'quatrieme'
        ensure_dir(back_cover_dir)
        destination = back_cover_dir / back_cover_path.name
        shutil.copy2(back_cover_path, destination)
        return destination
    ...
    def _render_credit_block(self, page: PageDef, site_meta: SiteMeta) -> str:
        ...
        if page_creators:
            suggestion.append(html.escape(" ; ".join(page_creators)))
        ...
        if volume_creators_text and set(page_creators) != set(site_meta.creators):
            host = (
                f'dans {html.escape(volume_creators_text)} '
                f'({html.escape(role_label.lower())}), '
                f'<em>{html.escape(volume_title)}</em>'
            )
        else:
