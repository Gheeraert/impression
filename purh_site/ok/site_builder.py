...
class SiteBuilder:
    ...
    def _finalize_build(
        ...
        back_cover_html = self._resolve_back_cover_html(tree, config.output_assets_dir)
        ...
        if back_cover_html:
            report_lines.append("Quatrième de couverture : détectée")
    ...
    def _write_index_page(
        ...
        if back_cover_html:
            hero_parts.append('<section class="home-panel home-panel--back-cover"><h2>Quatrième de couverture</h2>')
            hero_parts.append(f'<div class="back-cover-text">{back_cover_html}</div>')
    ...
    def _resolve_back_cover_html(self, tree: etree._ElementTree, output_assets_dir: Path) -> str | None:
        xml_html = self._extract_back_cover_from_xml(tree)
        if xml_html:
            return xml_html
        return self._read_back_cover_from_assets(output_assets_dir)
    ...
    def _read_back_cover_from_assets(self, output_assets_dir: Path) -> str | None:
        ...
        if txt_files:
            ...
            if not text:
                return None
            paragraphs = [
                f"<p>{self._render_simple_markdown_inline(block.strip())}</p>"
    ...
    def _render_simple_markdown(self, source_text: str) -> str:
        blocks = [block.strip() for block in re.split(r"\n\s*\n", source_text.strip()) if block.strip()]
        html_parts: list[str] = []
        for block in blocks:
            if block.startswith("### "):
                html_parts.append(f"<h3>{self._render_simple_markdown_inline(block[4:].strip())}</h3>")
            elif block.startswith("## "):
                html_parts.append(f"<h2>{self._render_simple_markdown_inline(block[3:].strip())}</h2>")
            elif block.startswith("# "):
                html_parts.append(f"<h1>{self._render_simple_markdown_inline(block[2:].strip())}</h1>")
            else:
                lines = [line.strip() for line in block.splitlines()]
                paragraph = " ".join(line for line in lines if line)
                html_parts.append(f"<p>{self._render_simple_markdown_inline(paragraph)}</p>")
        return "".join(html_parts)

    def _render_simple_markdown_inline(self, text_value: str) -> str:
