...
class SiteBuilder:
    ...
    def _write_content_page(
        ...
        page_header = [
            f'<header class="page-header"><p class="eyebrow">{html.escape(" / ".join(page.section_chain))}</p>'
            if page.section_chain
            else '<header class="page-header">'
        ]
        ...
        if page.author_entries:
            page_header.append(self._render_author_block(page.author_entries))
        elif page.authors:
    ...
    def _strip_redundant_title_pages(self, root: etree._Element) -> None:
        """Supprime les pages de titre internes déjà rendues par l'en-tête HTML."""
    ...
    def _render_credit_block(self, page: PageDef, site_meta: SiteMeta) -> str:
        if page.page_kind == 'article' and page.authors:
            citation = [
                html.escape(' ; '.join(page.authors)),
                f'« {html.escape(page.title)} »',
                f'dans {html.escape(site_meta.title)}',
            ]
