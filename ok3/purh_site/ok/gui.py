...
class App(ttk.Frame):
    ...
    def __init__(self, master: tk.Tk) -> None:
        ...
        self.back_cover_file_var = tk.StringVar()
    ...
    def _build_ui(self) -> None:
        ...
        self._add_path_selector(
            4,
            "Quatrième de couverture (optionnel : .md, .markdown, .html, .txt)",
            self.back_cover_file_var,
            self._choose_back_cover_file,
            "Choisir…",
        )
        self._add_path_selector(5, "Dossier de sortie", self.output_dir_var, self._choose_output_dir, "Choisir…")

        preview_bar = ttk.Frame(self)
        preview_bar.grid(row=6, column=0, columnspan=3, sticky="w", pady=(0, 8))
        ...
        ttk.Checkbutton(
            preview_bar,
            text="Lancer le serveur local et ouvrir le navigateur après build",
        ...
        helper = ttk.Label(
            ...
            text=(
                ...
                "Les chemins d’images indiqués dans le XML font foi ; logos conseillés : "
                "cover/couverture, universite/urn, purh. "
                "Pour la quatrième : priorité au XML, sinon le fichier choisi ici sera copié dans assets/quatrieme/."
            ),
        )
        helper.grid(row=7, column=0, columnspan=3, sticky="w", pady=(0, 8))

        button_bar = ttk.Frame(self)
        button_bar.grid(row=8, column=0, columnspan=3, sticky="w", pady=(8, 12))
    ...
    def _choose_back_cover_file(self) -> None:
        path = filedialog.askopenfilename(
            title="Choisir un fichier pour la quatrième de couverture",
            filetypes=[
                ("Markdown", "*.md *.markdown"),
                ("HTML", "*.html *.htm"),
                ("Texte", "*.txt"),
                ("Tous les fichiers pris en charge", "*.md *.markdown *.html *.htm *.txt"),
            ],
        )
        if path:
            self.back_cover_file_var.set(path)
            self._log(f"Fichier de quatrième : {path}")
    ...
    def _build(self) -> None:
        ...
        base_assets_dir = Path(self.assets_dir_var.get()).resolve() if self.assets_dir_var.get().strip() else None
        back_cover_file = Path(self.back_cover_file_var.get()).resolve() if self.back_cover_file_var.get().strip() else None
        master_xml_text = self.master_xml_var.get().strip()

        try:
            with tempfile.TemporaryDirectory(prefix="impressions-assets-") as temp_assets_root:
                assets_dir = self._prepare_assets_dir_for_build(
                    base_assets_dir=base_assets_dir,
                    back_cover_file=back_cover_file,
                    temp_root=Path(temp_assets_root),
                )
    ...
    def _prepare_assets_dir_for_build(
        self,
        base_assets_dir: Path | None,
        back_cover_file: Path | None,
        temp_root: Path,
    ) -> Path | None:
        if base_assets_dir is None and back_cover_file is None:
            return None

        staged_assets_dir = temp_root / "assets"
        staged_assets_dir.mkdir(parents=True, exist_ok=True)

        if base_assets_dir is not None:
            if not base_assets_dir.exists():
                raise FileNotFoundError(f"Dossier assets introuvable : {base_assets_dir}")
            for child in base_assets_dir.iterdir():
                destination = staged_assets_dir / child.name
                if child.is_dir():
        ...
        if back_cover_file is not None:
            if not back_cover_file.exists():
                raise FileNotFoundError(f"Fichier de quatrième introuvable : {back_cover_file}")
            suffix = back_cover_file.suffix.lower()
            if suffix not in {".md", ".markdown", ".html", ".htm", ".txt"}:
                raise ValueError("La quatrième doit être en .md, .markdown, .html, .htm ou .txt.")
            back_cover_dir = staged_assets_dir / "quatrieme"
            back_cover_dir.mkdir(parents=True, exist_ok=True)
            for existing in back_cover_dir.iterdir():
                if existing.is_file() and existing.suffix.lower() in {".md", ".markdown", ".html", ".htm", ".txt"}:
                    existing.unlink()
            shutil.copy2(back_cover_file, back_cover_dir / f"quatrieme{suffix}")
            self._log(f"Quatrième copiée pour le build : {back_cover_dir / f'quatrieme{suffix}'}")
